...
public class Program {
    ...
    public static void solve(Scanner scanner) {
        ...
        while (!pq.isEmpty()) {
            ...
            long cost = state[0];
            int u = (int) state[1];
            int portalUsed = (int) state[2];

            if (cost > dist[u][portalUsed]) continue;

            for (int[] edge : adj.get(u)) {
                int v = edge[0];
                int w = edge[1];

                long effectiveCost = w + (portalUsed == 0 ? curse[u] : 0);

                // Option A: do NOT use portal
                long newCostA = cost + effectiveCost;
                if (newCostA < dist[v][portalUsed]) {
                    dist[v][portalUsed] = newCostA;
                    pq.offer(new long[]{newCostA, v, portalUsed});
                }

                // Option B: USE portal at u (only if not yet used)
                if (portalUsed == 0) {
                    long effectivePortalCost = curse[u];
                    long newCostB = cost + effectivePortalCost;
                    if (newCostB < dist[v][1]) {
                        dist[v][1] = newCostB;
                        pq.offer(new long[]{newCostB, v, 1});
                    }
                }
            }
        }

        long answer = Math.min(dist[T][0], dist[T][1]);
